  ...
  useEffect(() => {
    const fetchUsers = async () => {
      ...
      if (error) {
        ...
        data.forEach((u) => (userMap[u.id] = u));
        setUsers(userMap);
        console.log('Fetched users:', userMap); // Log the users
      }
    };
      ...
      <div className="flex-1 overflow-y-auto">
        {chat &&
          chat.map((message, index) => {
            const sender = users[message.sender_id];
            if (!sender) {
              console.warn(`Sender not found for message ID: ${message.sender_id}`);
              return null;
            }
            return (
              <div
              ...
              >
                <div className="text-sm">
                  <div className="font-semibold mb-1">{sender?.name || 'Unknown'}</div>
                  {message.text}
                </div>
                <div className="text-xs text-gray-500">
                  {new Date(message.time).toLocaleTimeString()}
